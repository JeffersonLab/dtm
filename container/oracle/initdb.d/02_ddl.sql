...
(
    ...
    AFFECTED_HALL_YN     VARCHAR2(1 CHAR) DEFAULT 'N' NOT NULL,
    AFFECTED_RESEARCH_YN VARCHAR2(1 CHAR) DEFAULT 'N' NOT NULL,
    CONSTRAINT EVENT_TYPE_PK PRIMARY KEY (EVENT_TYPE_ID),
    CONSTRAINT EVENT_TYPE_CK1 CHECK (ARCHIVED_YN IN ('Y','N')),
    CONSTRAINT EVENT_TYPE_CK2 CHECK (AFFECTED_HALL_YN IN ('Y','N')),
    CONSTRAINT EVENT_TYPE_CK3 CHECK (AFFECTED_RESEARCH_YN IN ('Y','N'))
);
...
(
    ...
    CONSTRAINT INCIDENT_CK1 CHECK (EXPERT_ACKNOWLEDGED IN ('N', 'Y', 'R')),
    CONSTRAINT INCIDENT_CK2 CHECK (RESEARCH_YN IN ('Y','N'))
);
...
(
    ...
    ROOT_CAUSE           VARCHAR2(512 CHAR) NULL ,
    RAR_ID               INTEGER NULL ,
    EXPERT_ACKNOWLEDGED  CHAR(1) DEFAULT 'N' NOT NULL ,
    ...
    CONSTRAINT INCIDENT_AUD_CK1 CHECK (EXPERT_ACKNOWLEDGED IN ('N', 'Y', 'R')),
    CONSTRAINT INCIDENT_AUD_CK2 CHECK (RESEARCH_YN IN ('Y','N'))
);
...
CREATE TABLE DTM_OWNER.TYPE_CATEGORY (
    ...
    -- If using a VIEW for CATEGORY, Oracle isn't going to like creating this constraint, otherwise, use it
    -- ,TYPE_CATEGORY_FK2 FOREIGN KEY (CATEGORY_ID) REFERENCES DTM_OWNER.CATEGORY (CATEGORY_ID) ON DELETE CASCADE
);
