...
public class IncidentRepairTrendService {
  ...
  private static final Logger LOGGER = Logger.getLogger(IncidentRepairTrendService.class.getName());

  public List<HistogramBin> findTrendListByPeriodInMemory(RepairSummaryReportParams params)
      throws SQLException {
    ...
    // Note we only grab incidents that are closed for simplicity and to avoid issues with computing
    // duration
    String sql;
    ...
    sql =
        "select incident_id, time_down, time_up, cast(greatest(a.time_down, ?) as date) as time_down_bounded, cast(least(coalesce(a.time_up, sysdate), ?) as date) as time_up_bounded, "
    ...
    try {
      ...
      LOGGER.log(
          Level.FINEST, "Repair report query seconds: {0}", (queryEnd - queryStart) / 1000.0f);
      ...
      while (rs.next()) {
        ...
        if (BinSize.HOUR.equals(
            params
                .getBinSize())) { // We are doing an hourly histogram so use most recent top of hour
          // (truncate minutes/seconds/millis)
          binStart = TimeUtil.startOfHour(timeDown, Calendar.getInstance());
          binUnit = Calendar.HOUR_OF_DAY;
        } else if (BinSize.DAY.equals(
            params.getBinSize())) { // We are doing a daily histogram so truncate
        ...
        do {
          ...
          long durationMillis =
              Math.min(timeUp.getTime(), nextBinStart.getTime()) - timeDown.getTime();
      ...
      LOGGER.log(
          Level.FINEST,
          "Repair report binning seconds: {0}",
          (binningEnd - binningStart) / 1000.0f);
    ...
    for (Date dateKey : incidentMap.keySet()) { // Dates are already ordered...
    ...
    LOGGER.log(
        Level.FINEST, "Repair report sorting seconds: {0}", (sortingEnd - sortingStart) / 1000.0f);
  ...
  private void addIncident(
      int iteration,
      LinkedHashMap<Date, HashMap<String, HistogramBin>> incidentMap,
      String grouping,
      Date binDate,
      long durationMillis) {
