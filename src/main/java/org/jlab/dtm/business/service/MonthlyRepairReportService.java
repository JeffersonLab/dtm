...
public class MonthlyRepairReportService {
    ...
    public List<MonthlyRepairReportRecord> find(Date start, Date end) throws SQLException {
        ...
        try {
            ...
            // Step 1 is to get data from SQL then break apart intervals into chunks (some intervals = more than one chunk if they span multiple days)
            while (rs.next()) {
                ...
                List<DayChunk> intermediateChunkList = getChunks(timeDown, timeUp, category,
                        categoryId);
            ...
            for (DayChunk chunk : chunkList) {
                ...
                if (binBlock == null) {
                    binBlock = new MonthlyRepairReportRecord(category, categoryId, binDate,
                            downtimeHours);
        ...
        // Step 3 is to iterate over bins (days), sort the categories for each day, and create a flat list of records
    ...
    private List<DayChunk> getChunks(Date timeDown, Date timeUp, String category,
