...
public class FsdTripService {
  ...
  private static final Logger logger = Logger.getLogger(FsdTripService.class.getName());

  public BigInteger countList(FsdTripFilter filter) throws SQLException {
  ...
  public FsdSummary filterSummary(FsdTripFilter filter) throws SQLException {
    ...
    // Note: We grab count which START in user selected time period.  This is consistent with other
    // FSD reports, but differs from most other downtime and time accounting reports.
    String sql =
  ...
  public List<FsdTrip> filterListWithDependencies(FsdTripFilter filter, int offset, int max)
      throws SQLException {
    ...
    /**
     * There are at least two ways to apply a user specified time interval filter to a set of items
     * which have a duration themselves. (1) Include the item if it overlaps the time interval at
     * any point; could begin before the interval or extend after it. (2) Only include items which
     * begin within the interval. If doing the first then it might be helpful to take the greatest
     * of item start and boundary start as the psudo item start and to take the least of item end
     * and boundary end as the psudo item end. If doing the second it is easy to create a histogram
     * because you don't have to break up an item into multiple pieces depending on how many bins
     * the item spans.
     *
     * <p>This query behaves like the second as it is correlated with a histogram (FSD Summary
     * report)
     *
     * <p>Note: the operability annual repair report does the more difficult binning thing. Maybe
     * someday we can update this to do the same. However, this gets more complicated when you want
     * to know the number/count of trips too - does splitting a bin boundary cause the count to
     * increase?
     */
    String sql =
    ...
    sql =
        "select fsd_trip_id, fsd_fault_id, fsd_device_exception_id, start_utc, end_utc, acc_state, hla_state, hlb_state, hlc_state, hld_state, node, channel, disjoint_yn, category.name as category, hco_system_name, ced_type, ced_name, fault_confirmation_yn, cause, area, dtm_owner.region.name as region from ("
            + sql
            + ") left join fsd_fault using(fsd_trip_id) left join fsd_device_exception using(fsd_fault_id) left join dtm_owner.system s on s.name = hco_system_name left join system_alpha_category on s.system_id = system_alpha_category.system_id left join dtm_owner.category on system_alpha_category.category_id = category.category_id "
  ...
  private List<FsdTrip> populateTripsFromRs(ResultSet rs) throws SQLException {
    ...
    while (rs.next()) {
      ...
      AccMachineState accState = AccMachineState.valueOfAllowNull(rs.getString("acc_state"));
      HallMachineState hallAState = HallMachineState.valueOfAllowNull(rs.getString("hla_state"));
      HallMachineState hallBState = HallMachineState.valueOfAllowNull(rs.getString("hlb_state"));
      HallMachineState hallCState = HallMachineState.valueOfAllowNull(rs.getString("hlc_state"));
      HallMachineState hallDState = HallMachineState.valueOfAllowNull(rs.getString("hld_state"));
      String cause = rs.getString("cause");
      String area = rs.getString("area");

      // Fault fields
      BigInteger fsdFaultId =
          rs.getBigDecimal("fsd_fault_id") == null
              ? null
              : rs.getBigDecimal("fsd_fault_id").toBigIntegerExact();
      ...
      BigInteger fsdDeviceId =
          rs.getBigDecimal("fsd_device_exception_id") == null
              ? null
              : rs.getBigDecimal("fsd_device_exception_id").toBigIntegerExact();
  ...
  private List<FsdTrip> selectTripsWithNullCause() throws SQLException {
    ...
    String sql =
        "select fsd_trip_id, fsd_fault_id, fsd_device_exception_id, start_utc, end_utc, acc_state, hla_state, hlb_state, hlc_state, hld_state, node, channel, disjoint_yn, dtm_owner.category.name as category, hco_system_name, ced_type, ced_name, fault_confirmation_yn, cause, area, dtm_owner.region.name as region from fsd_trip "
            + "left join fsd_fault using(fsd_trip_id) left join fsd_device_exception using(fsd_fault_id) left join dtm_owner.system s on s.name = hco_system_name left join dtm_owner.system_alpha_category on s.system_id = dtm_owner.system_alpha_category.system_id left join dtm_owner.category on dtm_owner.system_alpha_category.category_id = dtm_owner.category.category_id "
            + "left join dtm_owner.component on fsd_device_exception.ced_name = component.name left join dtm_owner.region on region.region_id = component.region_id "
            + "where cause is null";
  ...
  private void updateTripListCause(List<FsdTrip> tripList) throws SQLException {
    ...
    String sql = "update fsd_trip set cause = ? where fsd_trip_id = ?";
  ...
  private void updateTripListArea(List<FsdTrip> tripList) throws SQLException {
    ...
    String sql = "update fsd_trip set area = ? where fsd_trip_id = ?";
  ...
  public void streamTripCsv(
      OutputStream out, FsdTripFilter filter, String message, boolean aggregate)
      throws SQLException {
     ...
     */
    String sql
        //   = "select * from fsd_trip a left join fsd_fault b on a.fsd_trip_id = b.fsd_trip_id left
        // join fsd_device_exception c on b.fsd_fault_id = c.fsd_fault_id ";
        =
    ...
    if (aggregate) {
      ...
      sql =
          sql
              + "and a.fsd_trip_id in (select fsd_trip_id from fsd_trip left join fsd_fault using(fsd_trip_id) left join fsd_device_exception using(fsd_fault_id) group by fsd_trip_id having count(fsd_trip_id) <= 100) ";

      sql =
          "select fsd_trip_id, start_utc, end_utc, acc_state, hla_state, hlb_state, hlc_state, hld_state, cause, area, "
              ...
              // + "rtrim(xmlagg(xmlelement(e,hco_system_name,'; ').extract('//text()') order by
              // hco_system_name).GetClobVal(),'; ') as hco_system_name, " // This is too slow, but
              // does solve the 4000 bytes problem
              ...
              + "listagg(fsd_device_exception_id, '; ') within group (order by fsd_device_exception_id) as fsd_device_exception_id from ("
              + sql
              + ") group by fsd_trip_id, start_utc, end_utc, acc_state, hla_state, hlb_state, hlc_state, hld_state, cause, area ";
    }
    ...
    try {
      ...
      try (PrintWriter writer = new PrintWriter(out)) {
        ...
        while (rs.next()) {
          ...
          Date end = rs.getTimestamp("end_utc", TimeUtil.getUtcCalendar());
          AccMachineState accState = AccMachineState.valueOfAllowNull(rs.getString("acc_state"));
          HallMachineState hallAState =
              HallMachineState.valueOfAllowNull(rs.getString("hla_state"));
          HallMachineState hallBState =
              HallMachineState.valueOfAllowNull(rs.getString("hlb_state"));
          HallMachineState hallCState =
              HallMachineState.valueOfAllowNull(rs.getString("hlc_state"));
          HallMachineState hallDState =
              HallMachineState.valueOfAllowNull(rs.getString("hld_state"));
          String area = rs.getString("area");
          String cause = rs.getString("cause");
