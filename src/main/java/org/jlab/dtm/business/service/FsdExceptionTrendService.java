...
public class FsdExceptionTrendService {
  ...
  private static final Logger logger = Logger.getLogger(FsdExceptionTrendService.class.getName());

  /**
   * This query is a doozy so I'm going to explain it.
   *
   * <p>Objective: Obtain a histogram of FSD device exceptions (count or duration sum) per unit time
   * (day or hour).
   *
   * <p>Note 1: The query must be dynamic based on user input so using a stored procedure or
   * database function becomes difficult (not to mention harder to maintain). Stored procedures /
   * database functions / anonymous PL/SQL blocks are also tricky when the goal is for it to return
   * a cursor as then the client must use an Oracle database driver specific API instead of JDBC.
   * Another route I examined was simply having the client issue multiple queries. This is ugly too,
   * and has terrible performance if doing one query per histogram 'bin'.
   *
   * <p>Note 2: There are two major confusing spots in this query: (1) we need to generate a
   * collection of histogram 'bins' - this is done by using 'select from dual... connect by'. (2) we
   * need to execute a complex correlated query for each bin that returns multiple columns, but a
   * correlated subquery can only return a single column - to get around this we create a single
   * string that is a concatenation of the three columns we need then parse the parts back later. An
   * alternative might be to issue multiple queries, one per column needed. Of course we could issue
   * one query per bin as well. Another alternative is to create a stored procedure that issues
   * PL/SQL loop and returns a cursor. None of the alternatives are great.
   *
   * <p>Note 3: Another complication with this query is that the database is storing the dates in
   * UTC, but we work in local time (America/New_York) in Java so you'll see 'from_tz ... at time
   * zone... as date'
   *
   * <p>Note 4: The 'least .... greatest' part is because the user is selecting a date range of
   * interest, but some of the FSD device exceptions may span a boundary.
   *
   * <p>Note 5: If the user indicates they want to group by category then the query gets even
   * hairier. Plus, the user can indicate to limit the results to only FSDs of a particular
   * category, and this is independent of whether the group by is in effect. The group by is further
   * complicated by the fact that we want a full list of categories for each bin, even if most are
   * zero; this is because the histogram needs to be continuous, especially if the user select line
   * mode instead of bar mode they would notice and it would be misleading. This generates a lot of
   * rows as it is logically a cross-join. This is done by using a 'union all ... group by date,
   * category'
   *
   * <p>Note 6: Parameter index to name map: 1 = start 2 = end 3 = start 4 = end 5 = start
   */
  ...
  public List<CategoryTrendInfo> findTrendByDateSingle(
      Date day, boolean groupByCategory, BigInteger[] categoryIdArray) throws SQLException {
    List<CategoryTrendInfo> trendList = new ArrayList<CategoryTrendInfo>();
  ...
  public List<CategoryTrendInfo> findTrendListByPeriodClientLoop(
      Date start, Date end, boolean groupByCategory, BigInteger[] categoryIdArray)
      throws SQLException {
  ...
  public List<CategoryTrendInfo> findTrendListByPeriodTableFunc(
      Date start, Date end, boolean groupByCategory, int interval, BigInteger[] categoryIdArray)
      throws SQLException {
