...
public class AnnualRepairReportService {
    ...
    private static final Logger logger = Logger.getLogger(
            AnnualRepairReportService.class.getName());
    ...
    public List<AnnualRepairReportRecord> find(Date start, Date end) throws SQLException {
        ...
        // Note 1: We query all incidents in the period and then do the group by in memory on the application server.  This makes it easier to group by a date interval.
        String sql = "select a.time_down, a.time_up, c.name as category, category_id "
        ...
        try {
            ...
            // Step 1 is to get data from SQL then break apart intervals into chunks (some intervals = more than one chunk if they span multiple months)
            while (rs.next()) {
                ...
                List<MonthChunk> intermediateChunkList = getChunks(timeDown, timeUp, category,
                        categoryId);
            ...
            for (MonthChunk chunk : chunkList) {
                ...
                double downtimeHours = (timeUp.getTime() - timeDown.getTime())
                        / MILLISECONDS_PER_HOUR;

                //SimpleDateFormat formatter = new SimpleDateFormat("MMM yyyy");
                //SimpleDateFormat formatter2 = new SimpleDateFormat(TimeUtil.getFriendlyDateTimePattern());
                //System.out.println("---------------------");
                ...
                if (binBlock == null) {
                    binBlock = new AnnualRepairReportRecord(category, categoryId, binDate,
                            downtimeHours);
        ...
        // Step 3 is to iterate over bins (months), sort the categories for each month, and create a flat list of records
    ...
    private List<MonthChunk> getChunks(Date timeDown, Date timeUp, String category,
