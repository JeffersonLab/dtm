...
public class RootCauseOverallMetricService {
  ...
  private static final Logger LOGGER =
      Logger.getLogger(RootCauseOverallMetricService.class.getName());

  public double countDurationSeconds(IncidentParams params) throws SQLException {
    ...
    String sql = "select sum(duration_seconds) from incident inner join event using(event_id) ";

    sql = sql + getSqlWhereClause(params);
  ...
  public BigInteger countRecords(IncidentParams params) throws SQLException {
    ...
    String sql = "select count(*) from incident inner join event using(event_id) ";
  ...
  public RootCauseOverallMetrics find(IncidentParams params) throws SQLException {
    ...
    RootCauseOverallMetrics metrics =
        new RootCauseOverallMetrics(
            repairHours,
            deadbeatHours,
            levelOneHours,
            levelTwoHours,
  ...
  private String getSqlWhereClause(IncidentParams params) {
    ...
    if (params.getSmeUsername() != null && !params.getSmeUsername().isEmpty()) {
      filters.add(
          "incident_id in (select incident_id from incident inner join incident_review using(incident_id) where reviewer_username = ?) ");
    }
  ...
  private void assignParameterValues(PreparedStatement stmt, IncidentParams params)
      throws SQLException {
    int i = 1;
  ...
  public static class RootCauseOverallMetrics {
    ...
    public RootCauseOverallMetrics(
        double repairHours,
        double deadbeatHours,
        double levelOneHours,
        double levelTwoHours,
        double levelThreePlusHours,
        long repairs,
        long deadbeats,
        long levelOne,
        long levelTwo,
