...
public class MigrateOldRarService {
  ...
  public List<OldRARRecord> getOldRecords(int year) throws SQLException {
    ...
    ResultSet rs = null;
    String sql =
        "select * from rer_owner.reports where request_date >= to_date('"
            + year
            + "-01', 'yyyy-mm') and request_date < to_date('"
            + (year + 1)
            + "-01', 'yyyy-mm') order by request_date asc";
  ...
  public AttachmentRecord getAttachment(int rarId) throws SQLException {
    ...
    ResultSet rs = null;
    String sql =
        "select file_name, mime_type, data from rer_owner.attachments where report_id = " + rarId;
