...
public class FsdTripTrendService {
  ...
  private static final Logger LOGGER = Logger.getLogger(FsdTripTrendService.class.getName());
  ...
  public List<TripHistogramBin> findTrendListByPeriodInMemory(FsdSummaryReportParams params)
      throws SQLException {
    ...
    LOGGER.log(
        Level.FINEST, "FSD report update cause seconds: {0}", (updateEnd - updateStart) / 1000.0f);
    ...
    // Note we only grab trips that are closed for simplicity and to avoid issues with computing
    // duration
    String sql;
    ...
    if (params.getMaxTypes() != null) {
      sql =
          "select fsd_trip_id, start_utc, end_utc, cause, area "
              + "from ("
    ...
    try {
      ...
      LOGGER.log(Level.FINEST, "FSD report query seconds: {0}", (queryEnd - queryStart) / 1000.0f);
      ...
      while (rs.next()) {
        ...
        if (BinSize.HOUR.equals(
          ...
          binDate =
              TimeUtil.startOfHour(
                  tripStart,
                  TimeUtil
                      .getUtcCalendar()); // Important to use UTC Calendar so that double 1 AM is
          // possible (truncate to hour in utc not local)
      ...
      LOGGER.log(
          Level.FINEST, "FSD report binning seconds: {0}", (binningEnd - binningStart) / 1000.0f);
    ...
    for (Date dateKey : tripMap.keySet()) { // Dates are already ordered...
    ...
    LOGGER.log(
        Level.FINEST, "FSD report sorting seconds: {0}", (sortingEnd - sortingStart) / 1000.0f);
