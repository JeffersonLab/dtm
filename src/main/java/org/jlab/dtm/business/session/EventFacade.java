...
public class EventFacade extends AbstractFacade<Event> {
    ...
    public Event findOpenEvent(BigInteger eventTypeId) {
        ...
        Root<Event> root = cq.from(Event.class);
        cq.select(root);
        cq.where(cb.and(cb.isNull(root.<Date>get("timeUp")), cb.equal(root.get("eventType"),
                eventTypeId)));
    ...
    @PermitAll
    public void editEvent(BigInteger eventId, Date timeUp, String title, BigInteger eventTypeId) throws
            WebApplicationException {
        logger.log(Level.FINEST,
                "editEvent - eventId: {0}, timeUp: {1}, title: {2}, eventTypeId: {3}", new Object[]{
                    eventId, timeUp, title, eventTypeId});
        checkAuthenticated();
        ...
        if (timeUp == null) { // Reopen or keep open
            ...
            if (openEvent != null && !event.equals(openEvent)) {
                throw new UserFriendlyException("An event of type " + event.getEventType().getName()
                        + " is already open");
            }
    ...
    public void validateEventTimeUp(Event event, Date timeUp) throws WebApplicationException {
        ...
        if (latestIncidentTimeUp != null && timeUp.before(latestIncidentTimeUp)) {
            throw new UserFriendlyException("Event time up (" + formatter.format(timeUp)
                    + ") must not be before latest incident time up (" + formatter.format(
                            latestIncidentTimeUp) + ")");
        }
    ...
    private Date computeLatestIncidentTimeUp(List<Incident> incidentList) {
        ...
        if (incidentList != null && !incidentList.isEmpty()) {
            ...
            for (Incident incident : incidentList) {
                ...
                } else if (incident.getTimeUp() != null && latestIncidentTimeUp.before(
                        incident.getTimeUp())) {
                    latestIncidentTimeUp = incident.getTimeUp();
                }
            }
    ...
    private Date computeLatestIncidentTimeDown(List<Incident> incidentList) {
        ...
        if (incidentList != null && !incidentList.isEmpty()) {
            ...
            for (Incident incident : incidentList) {
                ...
                } else if (incident.getTimeDown() != null && latestIncidentTimeDown.before(
                        incident.getTimeDown())) {
                    latestIncidentTimeDown = incident.getTimeDown();
                }
            }
        }
    ...
    public List<Event> filterList(AllEventsParams params) {
        ...
        if (params.getStart() != null) { // coalesce(time_up, sysdate) >= ?
            filters.add(cb.greaterThanOrEqualTo((cb.coalesce(event.get("timeUp"), new Date())),
                    params.getStart()));
        }
        ...
        List<EventTimeDown> eventTimeDownList
                = getEntityManager().createQuery(cq).setFirstResult(params.getOffset()).setMaxResults(params.getMax()).getResultList();
        ...
        if (event != null) {
            ...
            if (periodList != null) {
                for (Period p : periodList) {
                    restoreMillis = restoreMillis + (p.getEndDate().getTime()
    ...
    @PermitAll
    public List<Event> filterListWithIncidentsDefaultOpen(BigInteger eventTypeId, BigInteger eventId,
