...
public class EventDowntimeFacade extends AbstractFacade<Event> {
    ...
    private static final Logger LOGGER = Logger.getLogger(
            EventDowntimeFacade.class.getName());
    ...
    @PermitAll
    public List<EventDowntime> findByPeriodAndTypeSortByDuration(Date start, Date end,
            EventType type, Boolean beamTransport) {
        String sql
        ...
        if (beamTransport != null) {
            if (beamTransport) {
                ...
                sql = sql
                        + "where x.event_id not in (select event_id from incident where system_id = 616) ";
            }
        }

        sql = sql
                + "order by x.duration_bounded desc";
    ...
    public double downtimeTotal(Date start, Date end) {
        List<EventDowntime> downtimeList = findByPeriodAndTypeSortByDuration(start, end, EventType.ACC,
                null);
