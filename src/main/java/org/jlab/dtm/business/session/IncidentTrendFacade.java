...
public class IncidentTrendFacade extends AbstractFacade<Incident> {
    ...
    private static final Logger logger = Logger.getLogger(
            IncidentTrendFacade.class.getName());
    ...
    @PermitAll
    public List<CategoryTrendInfo> findTrendByDate(Date day, EventType type, boolean includeBeamTransport,
            boolean groupByCategory, BigInteger[] categoryIdArray) {
        String sql
        ...
        if (groupByCategory) {
            ...
            sql = "select day_date, sum(incident_count), sum(duration), category_name from ((" + sql + ") union all (select :start as day_date, 0 as incident_count, 0 as duration, category_name from (select distinct(y.name) as category_name from system_alpha_category x, category y where x.category_id = y.category_id " + categoryQualifier + "))) group by day_date, category_name ";
        }
    ...
    public List<CategoryTrendInfo> findTrendListByPeriod(Date start, Date end,
            EventType type, boolean includeBeamTransport, boolean groupByCategory, int interval, BigInteger[] categoryIdArray) {
        ...
        for (Date date : iterator) {
            trendList.addAll(findTrendByDate(date, type, includeBeamTransport, groupByCategory, categoryIdArray));
        }
