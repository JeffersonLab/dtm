...
public class SystemDowntimeFacade extends AbstractFacade<SystemEntity> {
    ...
    @PermitAll
    public List<SystemDowntime> findByPeriodAndType(Date start, Date end, EventType type,
            Boolean beamTransport, BigInteger categoryId, boolean packed) {
        
        
       // Note: If you don't choose an event_type_id (ACC, HALL A, etc.) then the packed interval still won't pack incidents from different types
        ...
        if (packed) {
            ...
            sql = sql
                    + "incident b inner join event c using(event_id), ";
        }
