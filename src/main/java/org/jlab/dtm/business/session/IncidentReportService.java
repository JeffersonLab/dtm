...
public class IncidentReportService {
  ...
  public static class IncidentSummary {
    ...
    public IncidentSummary(
      ...
      this.componentId = componentId == null ? null : BigInteger.valueOf(componentId.longValue());
  ...
  public List<IncidentSummary> filterList(IncidentDowntimeReportParams params) {
    ...
    if (start != null) {
      sql = sql + "cast(greatest(a.time_down, :start) as date) as time_down_bounded, ";
    } else {
      sql = sql + "cast(a.time_down as date) as time_down_bounded, ";
    }

    if (end != null) {
      sql = sql + "cast(least(coalesce(a.time_up, sysdate), :end) as date) as time_up_bounded, ";
    } else {
      sql = sql + "cast(coalesce(a.time_up, sysdate) as date) as time_up_bounded, ";
    }

    sql = sql + "(select count(incident_id) from incident where component_id = a.component_id ";
  ...
  public double sumTotalBoundedDuration(IncidentDowntimeReportParams params) {
    ...
    String sql = "select sum(interval_to_seconds(";
    ...
    if (groupId != null) {
      sql =
          sql
              + "and "
              + groupId
              + " in (select repaired_by from incident_repair where incident_id = a.incident_id)"
              + " ";
    }
    ...
    if (overnightOpened != null) {
      if (overnightOpened) {
        sql = sql + "and (to_char(a.time_down, 'HH24') < 7 or to_char(a.time_down, 'HH24') > 19) ";
      } else { // overnight closed
        sql = sql + "and (to_char(a.time_up, 'HH24') < 7 or to_char(a.time_up, 'HH24') > 19) ";
      }
    }
