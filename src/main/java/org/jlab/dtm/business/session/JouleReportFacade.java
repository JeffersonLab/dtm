...
public class JouleReportFacade {
    ...
    public List<JouleRecord> find(JouleReportParams params) throws IOException, InterruptedException {
        ...
        if("none".equals(size)) {
            JouleRecord record = createRecord(params.getStart(), params.getEnd(), maintenance, quality, scaler);
            ...
            DtmDateIterator iterator = new DtmDateIterator(params.getStart(), params.getEnd(), field, amount);
    ...
    private JouleRecord createRecord(Date start, Date end, Float maintenance, Float quality, Float scaler) throws IOException, InterruptedException {
        ...
        double deliveredTuningAndRestoreHours = (beamSummary.getRestoreSeconds() + beamSummary.getAccSeconds()) / 3600.0;

        double totalDeliveredHours = deliveredResearchHours + deliveredBeamStudiesHours + deliveredTuningAndRestoreHours;
    ...
    public Double fetchBudgetedHours(Date start, Date end) throws IOException, InterruptedException {
        ...
        HttpRequest request = HttpRequest.newBuilder()
                .GET()
                .uri(URI.create(System.getenv("PAC_SCHEDULE_SERVER_URL") + "/btm/rest/scheduled-count?start=" + startStr + "&end=" + endStr))
    ...
    @PermitAll
    public void exportAsCsv(ServletOutputStream out, List<JouleRecord> recordList, String selectionMessage) {
        try (PrintWriter writer = new PrintWriter(out)) {
            writer.println("# Bin, Research, Studies, Tuning & Restore, Delivered, Budgeted, Delivered Ratio, Failures, Scheduled, Research Ratio, Reliability");

            SimpleDateFormat dateFormatter = new SimpleDateFormat("yyyy-MM-dd");
            DecimalFormat decimalFormat = new DecimalFormat("#####0.0"); /*No commas in format because we are in CSV!*/
