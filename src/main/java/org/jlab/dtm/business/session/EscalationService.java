...
public class EscalationService {
  ...
  private static final Logger logger = Logger.getLogger(EscalationService.class.getName());

  public static final long TWO_HOUR_MILLIS = 7200000;

  @Resource TimerService timerService;

  @EJB BulletinBoardService bbService;

  @PermitAll
  public void startEscalationTimer(
      BigInteger eventId, String eventTitle, EscalationLevel level, long durationMillis) {
    logger.log(
        Level.FINE,
        "Starting Escalation Timer for eventId: {0}, with level: {1}, and durationMillis: {2}",
        new Object[] {eventId, level, durationMillis});
    if (eventId == null) {
    ...
    for (Timer timer : timerCollection) {
      ...
      if (o instanceof EscalationInfo) {
        ...
        if (info != null
            ...
            && event.getEventId().equals(info.getEventId())) {
          timer.cancel();
          break;
        }
      }
      ...
      switch (info.getEscalationLevel()) {
        case TWO_HOUR:
          ...
          startEscalationTimer(
              info.getEventId(), info.getEventTitle(), EscalationLevel.FOUR_HOUR, TWO_HOUR_MILLIS);
          bbService.postMessage(
              "Downtime two hour escalation needed for event: " + info.getEventTitle(),
              BulletinBoardPriority.MEDIUM);
          break;
        case FOUR_HOUR:
          logger.log(Level.FINE, "FOUR_HOUR Escalation Triggered: {0}", info);
          bbService.postMessage(
              "Downtime four hour escalation needed for event: " + info.getEventTitle(),
              BulletinBoardPriority.HIGH);
          break;
        default:
          throw new IllegalArgumentException(
              "Unknown escalation level: " + info.getEscalationLevel());
      }
    } catch (IOException e) {
  ...
  public void startEscalationTimer(Event event) {
    ...
    if (event.isClosed()) {
      logger.log(
          Level.FINE,
          "Skipping event {0} escalation timer because event is closed",
          event.getEventId());
    } else if (elapsedMillis < TWO_HOUR_MILLIS) {
      logger.log(
          Level.FINEST, "Using TWO_HOUR escalation level for eventId: {0}", event.getEventId());
      durationMillis = TWO_HOUR_MILLIS - elapsedMillis;

      startEscalationTimer(
          event.getEventId(), event.getTitle(), EscalationLevel.TWO_HOUR, durationMillis);
    } else if (elapsedMillis < (TWO_HOUR_MILLIS * 2)) {
      logger.log(
          Level.FINEST, "Using FOUR_HOUR escalation level for eventId: {0}", event.getEventId());
      durationMillis = (TWO_HOUR_MILLIS * 2) - elapsedMillis;

      startEscalationTimer(
          event.getEventId(), event.getTitle(), EscalationLevel.FOUR_HOUR, durationMillis);
    } else {
      logger.log(
          Level.FINE,
          "Skippping event {0} escalation timer since escalation times have already passed",
          event.getEventId());
    }
  }
