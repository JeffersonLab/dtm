...
public class ExcelAvailabilityTrendFacade {
    ...
    @PermitAll
    public void export(OutputStream out, List<Category> alphaCatList,
                       List<TrendRecord> records, String filters) throws IOException {
        ...
        for (TrendRecord data : records) {
            ...
            for (Category cat : alphaCatList) {
                ...
                c = row.createCell(i++);
                c.setCellStyle(floatStyle);
                c.setCellValue(dt.getIncidentCount() == 0 ? 0 : dt.getDuration()
                        / dt.getIncidentCount() * 24);
