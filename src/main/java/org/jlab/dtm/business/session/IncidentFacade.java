...
public class IncidentFacade extends AbstractFacade<Incident> {
  ...
  public List<Period> findGapsBetweenIncidents(BigInteger eventId) {
    ...
    String query
        // = "SELECT cast(start_date as date) as start_date, cast(end_date as date) as end_date FROM
        // (SELECT MAX(time_up) OVER (ORDER BY time_down) start_date, LEAD(time_down) OVER (ORDER BY
        // time_down) end_date FROM incident where event_id = :eventId) WHERE start_date <
        // end_date";
        =
  ...
  private void validateAndPopulateIncident(
      Incident incident,
      String title,
      String summary,
      String permitToWork,
    ...
    if (componentId != null) {
      ...
      if (eternalComponentList.size() != 1) {
        throw new UserFriendlyException(
            "Multiple components with that name found, please filter by Category/System and choose from auto-complete");
      }
  ...
  private void validateAndSetExpertReviewedBy(Incident incident, String[] expertUsernameArray)
    ...
    if (expertsChanged) {
      // When expert list changes we should reset acknowledged to No.  This catches reassign case
      // too!
      incident.setExpertAcknowledged(SystemExpertAcknowledgement.N);
    }
  ...
  public void clearReviewList(BigInteger incidentId) {
    Query q = em.createNativeQuery("delete from incident_review where incident_id = :incidentId");
  ...
  public Event addEvent(
    ...
    LOGGER.log(
        ...
        new Object[] {eventTypeId, eventTimeUp, timeDown, timeUp, title, componentId, eventTitle});
    checkAuthenticated();
    ...
    if (!reviewer
        && eventTimeUp != null
        && eventTimeUp.getTime() < ((new Date()).getTime() - MILLIS_PER_10_HOURS)) {
  ...
  public void addIncident(
    ...
    checkCanEditIncidentOrEvent(
        (reviewedBy != null && !reviewedBy.trim().isEmpty()) || event.containsAtLeastOneReview(),
        event);
  ...
  public List<Incident> filterList(IncidentParams params) {
    ...
    if (params.getTitle() != null && !params.getTitle().isEmpty()) {
      filters.add(
          cb.like(cb.upper(root.get("title")), "%" + params.getTitle().toUpperCase() + "%"));
    }
    ...
    TypedQuery<Incident> q =
        em.createQuery(cq).setMaxResults(params.getMax()).setFirstResult(params.getOffset());
  ...
  @PermitAll
  public void editSystemExpertReview(
      BigInteger incidentId, SystemExpertAcknowledgement acknowledgement, String rootCause) {
    checkAuthenticated();
  ...
  public List<String> findAllExpertsWithRecentUnreviewedIncidents(int numberOfHours) {
    Query q =
        em.createNativeQuery(
            "select unique reviewer_username from incident_review, incident where incident_review.incident_id = incident.incident_id and expert_acknowledged = 'N' and time_up is not null and time_up > :timeUp");
  ...
  public List<TransitionRecord> migrateOldRarRecords(int year)
      throws SQLException, WebApplicationException, IOException, InterruptedException {
  ...
  @RolesAllowed("dtm-admin")
  public void migrateOldRarAttachments(List<TransitionRecord> recordList)
