...
public class TrendReportFacade {
    ...
    private void fillInTrendCategories(TrendRecord record, Date start, Date end, EventType type) {
        record.categoryDowntimeList = categoryDowntimeFacade.findByPeriodAndType(start, end,
                type, null, true, null);
        ...
        for (Category alpha : alphaCatList) {
            if (record.downtimeMap.get(alpha.getCategoryId().longValue()) == null) {
                CategoryDowntime dt
                        = new CategoryDowntime(alpha.getName(), alpha.getCategoryId(), 0, 0);
    ...
    @PermitAll
    public TrendRecord load(Date start, Date end, EventType type, boolean includeCategories) throws SQLException {
        ...
        List<EventDowntime> eventList = eventDowntimeFacade.findByPeriodAndTypeSortByDuration(start,
                end, type,
                null);
        ...
        tripParams.setAccStateArray(
                new AccMachineState[]{AccMachineState.NULL, AccMachineState.DOWN,
                        AccMachineState.ACC, AccMachineState.MD, AccMachineState.RESTORE});
    ...
    public List<List<TrendRecord>> findMultiple(MultiTrendReportParams params) throws SQLException, UserFriendlyException {
        ...
        if (labelArray != null && labelArray.length > 0) {
            ...
            if (startArray == null || endArray == null || startArray.length != labelArray.length || endArray.length != labelArray.length) {
