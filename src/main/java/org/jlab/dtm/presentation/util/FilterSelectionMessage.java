...
public final class FilterSelectionMessage {
    ...
    public static String getReportMessage(Date start, Date end, EventType type, SystemEntity system,
            Category category, String dateFormat, String component, Boolean beamTransport,
            boolean packed) {
    ...
    public static String getDateRangeReportMessage(Date start, Date end, EventType type,
            SystemEntity system, Category category, String dateFormat, String component,
            Boolean beamTransport, String grouping, String data, boolean packed) {
        ...
        if (packed && "Category".equals(grouping)) { // In Category reports - Can't filter by transport if packed
            transportQualifier = "";
        }
        ...
        String message = typeQualifier + packedQualifier + dataQualifier + groupingQualifier
                + timeQualifier + categoryQualifier + systemQualifier + transportQualifier;
    ...
    public static String getIncidentTrendReportMessage(Date start, Date end, EventType type,
            boolean includeBeamTransport, String data, String grouping, int interval,
            List<String> categoryNameList) {

        //List<String> filters = new ArrayList<String>();
        SimpleDateFormat formatter;

        if (start.equals(TimeUtil.startOfDay(start, Calendar.getInstance())) && end.equals(
                TimeUtil.startOfDay(end, Calendar.getInstance(Locale.ENGLISH)))) {
        ...
        String message = intervalQualifier + typeQualifier + "Incident " + dataQualifier
                + groupingQualifier + "from " + formatter.format(start) + " to " + formatter.format(
                end) + " " + categoryQualifier + transportQualifier;
    ...
    public static String getFsdTrendReportMessage(Date start, Date end, String data, String grouping,
            int interval, List<String> categoryNameList) {

        SimpleDateFormat formatter;

        if (start.equals(TimeUtil.startOfDay(start, Calendar.getInstance())) && end.equals(
                TimeUtil.startOfDay(end, Calendar.getInstance()))) {
            formatter = new SimpleDateFormat("dd-MMM-yyyy");
        } else {
            formatter = new SimpleDateFormat(TimeUtil.getFriendlyDateTimePattern());
        }
        ...
        String message = intervalQualifier + "Faulted FSD Device " + dataQualifier
                + groupingQualifier + "from " + formatter.format(start) + " to " + formatter.format(
                end) + " " + categoryQualifier;

        return message;
    }

    public static List<String> getFsdTripReportFootnotes(Integer maxDuration,
            String maxDurationUnits, Integer maxTypes, boolean programBasis, double programHours,
            double periodHours, Boolean sadTrips, RootCause[] causeArray) {
        ArrayList<String> footnoteList = new ArrayList<>();

        if (maxDuration != null && maxDurationUnits != null) {
            String maxDurationQualifier = "Max Trip Duration: " + maxDuration + " "
                    + maxDurationUnits;
    ...
    public static String getFsdTripTrendReportMessage(Date start, Date end, Integer maxDuration,
            String maxDurationUnits, Integer maxTypes, String grouping, int interval,
            List<String> categoryNameList) {
        ...
        String message = dateRangeQualifier + " " + categoryQualifier + maxDurationQualifier
                + maxTypesQualifier;
