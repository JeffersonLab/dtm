...
public class MultiTrendReportUrlParamHandler implements UrlParamHandler<MultiTrendReportParams> {

  private final HttpServletRequest request;
  private final Date today;
  private final Date sevenDaysAgo;

  public MultiTrendReportUrlParamHandler(
  ...
  public MultiTrendReportParams convert() {
    ...
    try {
      ...
      if (labelArray != null && labelArray.length > 0) {
        ...
        if (startStrArray == null
            || startStrArray.length != labelArray.length
            || endStrArray == null
            || endStrArray.length != labelArray.length) {
  ...
  public String message(MultiTrendReportParams params) {
    ...
    if (params.getLabelArray() != null && params.getLabelArray().length > 0) {
      message =
          message
              + params.getLabelArray()[0]
              + " {"
              + TimeUtil.formatSmartRangeSeparateTime(
                  params.getStartArray()[0], params.getEndArray()[0])
              + "}";
      for (int i = 1; i < params.getLabelArray().length; i++) {
        message =
            message
                + ", "
                + params.getLabelArray()[i]
                + " {"
                + TimeUtil.formatSmartRangeSeparateTime(
                    params.getStartArray()[i], params.getEndArray()[i])
                + "}";
      }
    }
   ...
   */
  @Override
  public void redirect(HttpServletResponse response, MultiTrendReportParams params)
      throws IOException {
