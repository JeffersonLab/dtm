 ...
 * @author ryans
 */
@WebServlet(name = "EventsData", urlPatterns = {"/data/events"})
        ...
        if (errorReason == null) {
            ...
            if (eventList != null) {
                for (Event event : eventList) {
                    ...
                    evtBld.add("duration", DtmFunctions.millisToHumanReadable(event.getElapsedMillis(), false));
                    ...
                    evtBld.add("time_up", event.getTimeUp() == null ? "" : formatter.format(event.getTimeUp()));
                    ...
                    for (Incident incident : event.getIncidentList()) {
                        ...
                        incBld.add("time_up", incident.getTimeUp() == null ? "" : formatter.format(incident.getTimeUp()));
                        incBld.add("system_id", incident.getSystem().getSystemId());
                        incBld.add("component_id", incident.getComponent() == null ? BigInteger.ZERO : incident.getComponent().getComponentId());
