...
@WebServlet(
    name = "MonthlyRepair",
    urlPatterns = {"/operability/monthly-repair"})
public class MonthlyRepair extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
      ...
      try {
        ...
        if (currentData.missingList != null) {
          for (CategoryDowntime cd : currentData.missingList) {
            MonthlyRepairReportRecord mrrr =
                new MonthlyRepairReportRecord(
                    cd.getName(), BigInteger.valueOf(cd.getId()), start, 0);
    ...
    request
        .getRequestDispatcher("/WEB-INF/views/operability/monthly-repair.jsp")
  ...
  private void findDefaultGoals(
      MonthlyNote monthInfo,
      Map<Long, CategoryMonthlyGoal> goalMap,
