 ...
 * @author ryans
 */
@WebServlet(name = "ExcelEventList", urlPatterns = {"/excel/event-list.xlsx"})
public class ExcelEventList extends HttpServlet {

    @EJB
    ExcelEventListService excelService;
    @EJB
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String filters = FilterSelectionMessage.getReportMessage(start, end, type, null, null, null, null, beamTransport, false);

        // Note: Bounded means the selected time period boundaries are used as min and max time down and time up for events which span the period boundary
        
        List<EventDowntime> eventList = downtimeFacade.findByPeriodAndTypeSortByDuration(start, end, type, beamTransport);
