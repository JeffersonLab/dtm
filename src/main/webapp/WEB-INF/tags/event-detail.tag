        ...
        <span class="closed-by-panel">
            <c:choose>
                <c:when test="${event.timeUp ne null}">
                    <span>Closed by: </span><a title="Activity Audit" href="${pageContext.request.contextPath}/reports/activity-audit?eventId=${event.eventId}"><c:out value="${dtm:formatStaff(event.closedBy)}"/></a>
                </c:when>
                <c:otherwise>
    ...
    <table class="data-table stripped-table incident-table" data-event-type-id="${event.eventType.eventTypeId}">
        ...
        <tbody>
            <c:forEach items="${event.incidentList}" var="incident">
                <tr data-incident-id="${incident.incidentId}" data-solution="${fn:escapeXml(incident.resolution)}" data-repaired-by-id-csv="${incident.repairedByIdCsv}" data-reviewed-by-username-ssv="${incident.reviewedByUsernameSsv}" data-repaired-by-formatted="${incident.repairedByIdCsv != null ? dtm:formatGroupList(incident.repairedByIdCsv, groupList) : '--None--'}" data-reviewed-by="${fn:escapeXml(incident.reviewedBy.username)}" data-reviewed-by-formatted="${incident.reviewedBy != null ? dtm:formatStaff(incident.reviewedBy) : ''}" data-reviewed-by-experts-formatted-tsv="${incident.reviewedByExpertsFormattedTsv}" data-acknowledged="${fn:escapeXml(incident.expertAcknowledged)}" data-root-cause="${fn:escapeXml(incident.rootCause)}" data-rar-id="${incident.rarId}" data-rar-ext="${incident.rarExt}" data-review-level="${incident.reviewLevelString}">
                    <td>
                        <span>
